...
Payment.prototype.refund = function(params, callback) {
  ...
  return this.request(pkg, {type: 'refund', needs, cert: true}, callback);
};
...
Payment.prototype.transfers = function(params, callback) {
  ...
  return this.request(pkg, {type: 'transfers', needs, cert: true}, callback);
};
...
Payment.prototype.transfersQuery = function(params, callback) {
  ...
  return this.request(pkg, {type: 'gettransferinfo', needs, cert: true}, callback);
};
...
Payment.prototype.sendRedpack = function(params, callback) {
  ...
  return this.request(pkg, {type: 'sendredpack', needs, cert: true}, callback);
};
...
Payment.prototype.sendGroupRedpack = function(params, callback) {
  ...
  var needs = ['mch_billno', 'send_name', 're_openid', 'total_amount', 'wishing', 'act_name', 'remark'];
  if (pkg.total_amount > 200) needs.push('scene_id');
  return this.request(pkg, {type: 'sendgroupredpack', needs, cert: true}, callback);
};
...
Payment.prototype.redpackQuery = function(params, callback) {
  ...
  return this.request(pkg, {type: 'gethbinfo', needs, cert: true}, callback);
};
...
Payment.prototype._validate = function(xml, type, callback) {
  if (this.debug) console.log('xml data:\n' + xml);
  var that = this;
  util.parseXML(xml, function (err, json) {
    if (err) {
      if (type === 'downloadbill') return callback(null, xml);
      return callback('XMLDataError', xml);
    }
    if (type === 'downloadbill') return callback(json.return_msg || 'XMLDataError');
    if (json.return_code !== 'SUCCESS') return callback(json.return_msg || 'XMLDataError');
    if (json.result_code !== 'SUCCESS') return callback(json.err_code || 'XMLDataError');
    switch (type) {
      case 'transfers':
      ...
      default:
        ...
        if (json.sign !== that.getSign(json)) return callback('sign签名不匹配');
    }
    callback(null, json);
  });
};
